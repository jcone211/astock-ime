...
| 2 | `*ST美丽` → 编码 `*STml`，深蓝把这类条目**静默过滤**，5552 只里 95 只风险警示股全部丢失（旧 `astock.dat` 回读只有 **5444 条、0 条带 `*`**） | 编码去掉 `*` → `stml`；另外给深蓝一份「词面也去掉前导符号」的清洗副本，新 `ms_pinyin_astock.dat` 回读 **5552/5552** |
| 3 | `万科A` → `wkA`（大小写混在编码里，输入习惯不一致，还容易和 `wka` 撞出两套码）；旧 `astock.dat` 里有 **146 个编码带大写字母** | 编码统一小写：`wka` / `szya` / `tclkj` |
