...
> 暂不支持：**微信输入法**与**讯飞输入法**。截至 2026.09.04，这两家的「自定义短语 / 常用语」
> 只能在设置界面里手动添加，没有批量导入配置文件的入口（微信输入法连词库导入也没有），
> 所以本项目不为它们产出词库。哪天放开批量导入，在 `src/astock_ime/phrase.py` 里加一个对应格式的 writer 就行。

zip 里的 `README-导入说明.txt` 是同一份说明的纯文本版；细节（回滚、手机端、暂不支持清单、常见问题）见
