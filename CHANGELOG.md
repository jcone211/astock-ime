...
* **目标输入法收敛到两个**：只保留有批量导入通道的微软拼音与搜狗，删掉为其它场景准备的多余产物（纯词表、备用短语写法）。
* **搜狗走自定义短语**（实测可用）：设置 → 输入 → 自定义短语 → 直接编辑配置文件，把 `custom_phrase_astock.txt` 内容整体替换进 `PhraseEdit.txt`；
  因此不再产出 `sogou_astock.txt`（GBK 文本词库）与 `custom_phrase_astock_alt.txt`。
