...
* **只保留支持批量导入的目标**：去掉一个只能手动逐条添加、没有词库/短语批量入口的通道
  （产物、文档、Release 说明里相关内容一并清理）。
* 新增 `--stocks`：自选股清单过滤（股票名 / `000001` / `600519.SH` 混写都行，支持 `@文件` 与 `# 注释`，
  输出保持你书写的顺序），配合 `dist/astock_words.txt`（纯词表，逐条抄写用）。
* 纯词表产物改名 `astock_words.txt`；`--freq hot`、`--code-alias` 等规则不变。
